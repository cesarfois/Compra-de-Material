...
export const authService = {
    ...
    /**
     * Authenticates using the Backend service account token (bypasses OAuth redirect)
     */
    loginWithServiceAccount: async () => {
        try {
            console.log('🔑 Logging in with Service Account via Backend...');
            const proxyBase = getProxyBaseUrl();
            const response = await axios.get(`${proxyBase}/api/auth/token`);
            const { token, url, tokenEndpoint } = response.data;

            if (!token) throw new Error('Nenhum token retornado pelo servidor backend.');

            const authData = {
                token,
                url: url || 'https://rcsangola.docuware.cloud',
                tokenEndpoint: tokenEndpoint
            };
            ...
            console.error('❌ Service account login failed:', error);
            throw error;
        }
    },
